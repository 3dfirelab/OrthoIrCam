...
def sky_median_sig_clip(input_arr, sig_fract, percent_fract, max_iter=100):
	...
	iteration = 0
	while ((old_div(math.fabs(old_sky - new_sky),new_sky)) > percent_fract) and (iteration < max_iter) :
		iteration += 1
		old_sky = new_sky
		sig = work_arr.std()
		upper_limit = old_sky + sig_fract * sig
...
def linear(inputArray, scale_min=None, scale_max=None):
	...
	imageData = old_div((imageData -scale_min), (scale_max - scale_min))
...
def sqrt(inputArray, scale_min=None, scale_max=None):
	...
	imageData = old_div(imageData, math.sqrt(scale_max - scale_min))
...
def log(inputArray, scale_min=None, scale_max=None):
	...
	try :
		imageData[indices1] = old_div(numpy.log10(imageData[indices1]),factor)
	except :
...
def asinh(inputArray, scale_min=None, scale_max=None, non_linear=2.0):
	...
	if scale_max == None:
		scale_max = imageData.max()
	factor = numpy.arcsinh(old_div((scale_max - scale_min),non_linear))
	indices0 = numpy.where(imageData < scale_min)
	indices1 = numpy.where((imageData >= scale_min) & (imageData <= scale_max))
	indices2 = numpy.where(imageData > scale_max)
	imageData[indices0] = 0.0
	imageData[indices2] = 1.0
	imageData[indices1] = old_div(numpy.arcsinh(old_div((imageData[indices1] - \
